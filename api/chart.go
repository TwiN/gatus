...
func ResponseTimeChart(cfg *config.Config) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if err != nil {
			if errors.Is(err, common.ErrEndpointNotFound) {
				if proxied, proxyErr := proxyRemoteEndpoint(c, cfg, key, "/response-times/"+duration+"/chart.svg"); proxied || proxyErr != nil {
					return proxyErr
				}
...
func ResponseTimeHistory(cfg *config.Config) fiber.Handler {
	return func(c *fiber.Ctx) error {
		duration := c.Params("duration")
		var from time.Time
		switch duration {
		...
		endpointKey, err := decodeEndpointKeyParam(c)
		if err != nil {
			return c.Status(400).SendString("invalid key encoding")
		}
		...
		if err != nil {
			if errors.Is(err, common.ErrEndpointNotFound) {
				if proxied, proxyErr := proxyRemoteEndpoint(c, cfg, endpointKey, "/response-times/"+duration+"/history"); proxied || proxyErr != nil {
					return proxyErr
				}
				return c.Status(404).SendString(err.Error())
			}
