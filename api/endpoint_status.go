...
func getEndpointStatusesFromRemoteInstances(remoteConfig *remote.Config) ([]*endpoint.EndpointStatus, error) {
	if remoteConfig == nil || len(remoteConfig.Instances) == 0 {
		return nil, nil
	}
	var endpointStatusesFromAllRemotes []*endpoint.EndpointStatus
	...
	for _, instance := range remoteConfig.Instances {
		...
		var endpointStatuses []*endpoint.EndpointStatus
...
// EndpointStatus retrieves a single endpoint.EndpointStatus by group and endpoint name
