...
func UptimeRaw(cfg *config.Config) fiber.Handler {
	return func(c *fiber.Ctx) error {
		...
		if err != nil {
			if errors.Is(err, common.ErrEndpointNotFound) {
				if proxied, proxyErr := proxyRemoteEndpoint(c, cfg, key, "/uptimes/"+duration); proxied || proxyErr != nil {
					return proxyErr
				}
...
func ResponseTimeRaw(cfg *config.Config) fiber.Handler {
	return func(c *fiber.Ctx) error {
		duration := c.Params("duration")
		var from time.Time
		switch duration {
		...
		if err != nil {
			if errors.Is(err, common.ErrEndpointNotFound) {
				if proxied, proxyErr := proxyRemoteEndpoint(c, cfg, key, "/response-times/"+duration); proxied || proxyErr != nil {
					return proxyErr
				}
				return c.Status(404).SendString(err.Error())
			} else if errors.Is(err, common.ErrInvalidTimeRange) {
