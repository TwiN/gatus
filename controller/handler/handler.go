...
func CreateRouter(staticFolder string, cfg *config.Config) *mux.Router {
	...
	if cfg != nil {
		...
		unprotected.HandleFunc("/v1/services/{key}/response-times/{duration}/badge.svg", ResponseTimeBadge(cfg)).Methods("GET")
		// SPA
		router.HandleFunc("/services/{name}", SinglePageApplication(staticFolder, cfg.UI)).Methods("GET") // XXX: Remove this in v4.0.0
