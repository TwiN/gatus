...
func (provider *AlertProvider) buildHTTPRequest(ep *endpoint.Endpoint, alert *alert.Alert, resolved bool, result *endpoint.Result) *http.Request {
	...
	body = strings.ReplaceAll(body, "[ENDPOINT_ERRORS]", strings.Join(result.Errors, "\n"))
	url = strings.ReplaceAll(url, "[ENDPOINT_ERRORS]", strings.Join(result.Errors, "\n"))
	if resolved {
...
func (provider *AlertProvider) Send(ep *endpoint.Endpoint, alert *alert.Alert, result *endpoint.Result, resolved bool) error {
	request := provider.buildHTTPRequest(ep, alert, resolved, result)
