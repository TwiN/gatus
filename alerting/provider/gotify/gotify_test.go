...
func TestAlertProvider_IsValid(t *testing.T) {
	scenarios := []struct {
		...
		{
			name:     "valid",
			provider: AlertProvider{ServerURL: "https://gotify.example.com", AppToken: "faketoken"},
			expected: true,
		},
		{
			name:     "invalid-server-url",
			provider: AlertProvider{ServerURL: "", AppToken: "faketoken"},
			expected: false,
		},
		{
			name:     "invalid-app-token",
			provider: AlertProvider{ServerURL: "https://gotify.example.com", AppToken: ""},
			expected: false,
		},
		{
...
func TestAlertProvider_buildRequestBody(t *testing.T) {
	...
	scenarios := []struct {
		...
		{
			Name:         "custom-title",
			Provider:     AlertProvider{ServerURL: "https://gotify.example.com", AppToken: "faketoken", Title: "custom-title"},
			Alert:        alert.Alert{Description: &description, SuccessThreshold: 5, FailureThreshold: 3},
			Resolved:     false,
