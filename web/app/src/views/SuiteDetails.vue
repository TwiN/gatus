  ...
  <div class="suite-details-container bg-background min-h-screen">
    <div class="container mx-auto px-4 py-8 max-w-7xl">
      ...
      <div v-else-if="!suite" class="text-center py-20">
        ...
        <h3 class="text-lg font-semibold mb-2">{{ t('suiteDetails.suiteNotFoundTitle') }}</h3>
        <p class="text-muted-foreground">{{ t('suiteDetails.suiteNotFoundDescription') }}</p>
      </div>
            ...
            <CardTitle>
              {{
                selectedResult?.timestamp === sortedResults[0]?.timestamp
                  ? t('suiteDetails.latestExecution')
                  : t('suiteDetails.executionAt', { time: formatTimestamp(selectedResult.timestamp) })
              }}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div class="space-y-4">
              ...
              <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                  ...
                  <p class="text-lg font-medium">{{ latestResult.success ? t('flow.success') : t('flow.failed') }}</p>
                </div>
                <div>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div v-else class="text-center py-8 text-muted-foreground">
              {{ t('suiteDetails.noExecutionHistoryAvailable') }}
            </div>
          </CardContent>
        </Card>
      </div>
