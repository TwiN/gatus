            ...
            <div>
              ...
              <div class="flex items-center gap-3 text-muted-foreground mt-2">
                <span v-if="endpointStatus.group">
                  {{ t('endpointDetails.groupLabel') }} {{ endpointStatus.group }}
                </span>
            ...
            <Card>
              ...
              <CardContent>
                <div class="text-2xl font-bold">
                  {{ currentHealthStatus === 'healthy' ? t('endpointDetails.operational') : t('endpointDetails.issuesDetected') }}
                </div>
              </CardContent>
            </Card>
              ...
              <Card v-for="period in ['30d', '7d', '24h', '1h']" :key="period">
                <CardHeader class="pb-2">
                  <CardTitle class="text-sm font-medium text-muted-foreground text-center">
                    {{
                      period === '30d'
                        ? t('endpointDetails.last30Days')
                        : period === '7d'
                          ? t('endpointDetails.last7Days')
                          : period === '24h'
                            ? t('endpointDetails.last24Hours')
                            : t('endpointDetails.lastHour')
                    }}
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <img
                    ...
                    :alt="t('endpointDetails.periodResponseTimeAlt', { period })"
                    class="mx-auto mt-2"
                  />
                </CardContent>
              </Card>
              ...
              <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <div v-for="period in ['30d', '7d', '24h', '1h']" :key="period" class="text-center">
                  ...
                  <img
                    ...
                    :alt="t('endpointDetails.periodUptimeAlt', { period })"
                    class="mx-auto"
                  />
                </div>
              </div>
        ...
        for (let i = data.events.length - 1; i >= 0; i--) {
          ...
          if (i === data.events.length - 1) {
            ...
            if (event.type === 'HEALTHY') {
              ...
              if (nextEvent) {
                event.fancyText = t('endpointDetails.endpointWasUnhealthyFor', {
