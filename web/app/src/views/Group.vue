...
export default {
  ...
  methods: {
    fetchData() {
      fetch(`${SERVER_URL}/api/v1/groups/${this.$route.params.group}/statuses?page=${this.currentPage}`, {credentials: 'include'})
      .then(response => {
        ...
        if (response.status === 200) {
          ...
          response.text().then(text => {
            console.log(`[Group][fetchData] Error: ${text}`);
          });
        }
      });
    },
