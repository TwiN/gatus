...
func TestNewStore(t *testing.T) {
	if _, err := NewStore("", "TestNewStore.db", testRetentionDays); err != ErrDatabaseDriverNotSpecified {
		t.Error("expected error due to blank driver parameter")
	}
	if _, err := NewStore("sqlite", "", testRetentionDays); err != ErrFilePathNotSpecified {
		t.Error("expected error due to blank path parameter")
	}
	if store, err := NewStore("sqlite", t.TempDir()+"/TestNewStore.db", testRetentionDays); err != nil {
...
func TestStore_InsertCleansUpOldUptimeEntriesProperly(t *testing.T) {
	store, _ := NewStore("sqlite", t.TempDir()+"/TestStore_InsertCleansUpOldUptimeEntriesProperly.db", testRetentionDays)
	defer store.Close()
	...
	// one should be deleted since they both surpass uptimeRetentionDays
	store.Insert(&testEndpoint, &core.Result{Timestamp: now.Add(-(uptimeCleanUpThreshold + time.Hour)), Success: true})
...
func TestStore_InsertCleansUpEventsAndResultsProperly(t *testing.T) {
	store, _ := NewStore("sqlite", t.TempDir()+"/TestStore_InsertCleansUpEventsAndResultsProperly.db", testRetentionDays)
	defer store.Close()
...
func TestStore_Persistence(t *testing.T) {
	...
	store, _ := NewStore("sqlite", file, testRetentionDays)
	store.Insert(&testEndpoint, &testSuccessfulResult)
	store.Insert(&testEndpoint, &testUnsuccessfulResult)
...
func TestStore_Save(t *testing.T) {
	store, _ := NewStore("sqlite", t.TempDir()+"/TestStore_Save.db", testRetentionDays)
	defer store.Close()
	if store.Save() != nil {
...
func TestStore_SanityCheck(t *testing.T) {
	store, _ := NewStore("sqlite", t.TempDir()+"/TestStore_SanityCheck.db", testRetentionDays)
	defer store.Close()
	store.Insert(&testEndpoint, &testSuccessfulResult)
...
func TestStore_InvalidTransaction(t *testing.T) {
	store, _ := NewStore("sqlite", t.TempDir()+"/TestStore_InvalidTransaction.db", testRetentionDays)
	defer store.Close()
	tx, _ := store.db.Begin()
	tx.Commit()
...
func TestStore_NoRows(t *testing.T) {
	store, _ := NewStore("sqlite", t.TempDir()+"/TestStore_NoRows.db", testRetentionDays)
	defer store.Close()
	tx, _ := store.db.Begin()
	defer tx.Rollback()
...
func TestStore_BrokenSchema(t *testing.T) {
	store, _ := NewStore("sqlite", t.TempDir()+"/TestStore_BrokenSchema.db", testRetentionDays)
	defer store.Close()
