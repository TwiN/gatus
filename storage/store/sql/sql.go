...
type Store struct {
	driver, file        string
	uptimeRetentionDays time.Duration
...
func NewStore(driver, path string, uptimeRetentionDays int) (*Store, error) {
	...
	store := &Store{
		driver:              driver,
		file:                path,
		uptimeRetentionDays: time.Duration(uptimeRetentionDays) * 24 * time.Hour,
	}
	var err error
...
func (s *Store) Insert(endpoint *core.Endpoint, result *core.Result) error {
	...
	if err != nil {
		...
		if ageOfOldestUptimeEntry > uptimeCleanUpThreshold {
			if err = s.deleteOldUptimeEntries(tx, endpointID, time.Now().Add(-(s.uptimeRetentionDays + time.Hour))); err != nil {
