...
#### Status
![Status](https://status.twin.sh/api/v1/endpoints/core_blog-external/status/badge.svg)

The path to generate a badge is the following:
```
/api/v1/endpoints/{key}/status/badge.svg
```
Where:
- `{key}` has the pattern `<GROUP_NAME>_<ENDPOINT_NAME>` in which both variables have ` `, `/`, `_`, `,` and `.` replaced by `-`.
...
the URL would look like this:
```
https://example.com/api/v1/endpoints/core_frontend/status/badge.svg
```
