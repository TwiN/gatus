...
func TestEval(t *testing.T) {
	...
	scenarios := []Scenario{
		...
		{
			Name:                 "query-selects-field",
			Path:                 `components.#(name=="B").status`,
			Data:                 `{"components": [{"name": "A", "status": "operational"}, {"name": "B", "status": "partial_outage"}]}`,
			ExpectedOutput:       "partial_outage",
			ExpectedOutputLength: 14,
			ExpectedError:        false,
		},
		{
			Name:                 "query-with-no-remaining-path-returns-whole-element",
			Path:                 `components.#(name=="B")`,
			Data:                 `{"components": [{"name": "A", "status": "operational"}, {"name": "B", "status": "partial_outage"}]}`,
			ExpectedOutput:       `{"name":"B","status":"partial_outage"}`,
			ExpectedOutputLength: 38,
			ExpectedError:        false,
		},
		{
			Name:                 "query-value-with-spaces-and-punctuation",
			Path:                 `components.#(name=="Outbound Calling Services - United States").status`,
			Data:                 `{"components": [{"name": "Outbound Calling Services - United States", "status": "operational"}]}`,
			ExpectedOutput:       "operational",
			ExpectedOutputLength: 11,
			ExpectedError:        false,
		},
		{
			Name:                 "query-no-match-is-an-error",
			Path:                 `components.#(name=="Nonexistent").status`,
			Data:                 `{"components": [{"name": "A", "status": "operational"}]}`,
			ExpectedOutput:       "",
			ExpectedOutputLength: 0,
			ExpectedError:        true,
		},
		{
			Name:                 "query-against-empty-array-is-an-error",
			Path:                 `components.#(name=="A").status`,
			Data:                 `{"components": []}`,
			ExpectedOutput:       "",
			ExpectedOutputLength: 0,
			ExpectedError:        true,
		...
		{
			Name:                 "query-unquoted-value-is-an-error",
			Path:                 `components.#(name==A).status`,
			Data:                 `{"components": [{"name": "A", "status": "operational"}]}`,
			ExpectedOutput:       "",
			ExpectedOutputLength: 0,
		...
		{
			Name:                 "query-missing-operator-is-an-error",
			Path:                 `components.#(name).status`,
			Data:                 `{"components": [{"name": "A", "status": "operational"}]}`,
			ExpectedOutput:       "",
			ExpectedOutputLength: 0,
			ExpectedError:        true,
