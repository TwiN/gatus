...
func initializeStorage(cfg *config.Config) {
	...
	for _, ep := range cfg.Endpoints {
		...
		visibility = append(visibility, endpoint.NewEndpointStatusVisibility(key, ep.Visibility.Public))
	}
	for _, ee := range cfg.ExternalEndpoints {
		key := ee.Key()
		keys = append(keys, key)
		visibility = append(visibility, endpoint.NewEndpointStatusVisibility(key, ee.Visibility.Public))
	}
	...
	for _, s := range cfg.Suites {
		suiteVisibility = append(suiteVisibility, suite.NewSuiteStatusVisibility(s.Key(), s.Visibility.Public))
	}
